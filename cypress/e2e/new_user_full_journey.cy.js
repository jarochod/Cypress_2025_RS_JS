...
import { PaymentPage } from "./page-objects/PaymentPage"; // ✅ import PaymentPage
import { deliveryDetails as userAddress } from "../fixtures/deliveryDetails";
import { paymentDetails } from "../fixtures/paymentDetails"; // ✅ dodany import paymentDetails
...
describe("New user full end-to-end test journey", () => {
  it("should complete the new user journey", () => {
    ...
    const paymentPage = new PaymentPage(); // ✅ obiekt PaymentPage
    ...
    productsPage.getProductPrices().then((prices) => {
      const sorted = productsPage.sortNumbers(prices);
    ...
    // [AKCJA] uzupełniamy dane adresowe
    ...
    // ✅ walidacja kwot za pomocą getValuesAfterActivateDiscount()
    paymentPage.getValuesAfterActivateDiscount().then((values) => {
      expect(values.discountedValue).to.be.lessThan(values.totalValue);
    });

    // ✅ uzupełnienie danych płatności
    paymentPage.fillPaymentDetails(paymentDetails);

    // ✅ finalizacja
    paymentPage.completePayment();

    // [ASERCJA] końcowy URL strony podziękowania
